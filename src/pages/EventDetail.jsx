...
import { EventStatusBadge, TaskStatusBadge, PriorityBadge, ContactStatusBadge } from '../components/StatusBadge'
...
export default function EventDetail() {
  ...
  const [resultForm, setResultForm] = useState({
    student_applied: '', company_applied: '', student_actual: '', company_actual: '',
  })
  ...
  const handleDeleteEvent = async () => {
    if (!confirm(`「${event?.name}」を削除しますか？タスクも合わせて削除されます。`)) return
    try {
    ...
    try {
      ...
      if (evResult) {
        ...
        await appendRow('results', [
          generateId(), id,
          resultForm.student_applied, resultForm.company_applied,
          resultForm.student_actual, resultForm.company_actual,
          now,
        ])
      }
      reloadResults()
  ...
  if (!event) {
    return (
      <div className="p-6 text-center text-gray-400 text-sm">
        {events.length === 0 ? '読み込み中...' : 'イベントが見つかりません'}
      </div>
    )
  }
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            {event.big_cat} › {event.mid_cat} › {event.small_cat}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            className="text-xs py-1.5 px-3 rounded border border-red-200 text-red-500 hover:bg-red-50"
          ...
          <button
            ...
            onClick={() => {
              setResultForm(evResult ? {
                student_applied: evResult.student_applied,
                company_applied: evResult.company_applied,
                student_actual: evResult.student_actual,
                company_actual: evResult.company_actual,
              } : resultForm)
        ...
        {editingResult ? (
          <div className="grid grid-cols-4 gap-3">
            {['student_applied', 'company_applied', 'student_actual', 'company_actual'].map((f, i) => (
              <div key={f}>
                ...
                <input
                  ...
                  value={resultForm[f]}
                  onChange={e => setResultForm(prev => ({ ...prev, [f]: e.target.value }))}
                  min="0"
                />
              </div>
            ))}
            <div className="col-span-4">
              <button
                className="px-4 py-1.5 rounded text-sm font-semibold text-gray-900 hover:opacity-90 disabled:opacity-50"
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: '学生申込', value: evResult?.student_applied },
              { label: '企業申込', value: evResult?.company_applied },
              { label: '学生参加実数', value: evResult?.student_actual },
              { label: '企業参加実数', value: evResult?.company_actual },
            ].map(({ label, value }) => (
...
function GanttChart({ tasks, ganttData, today }) {
  ...
  while (cur <= end) {
    const pos = getPos(cur.toISOString().split('T')[0])
    monthLabels.push({
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        {eventPos !== null && (
          <div
            ...
            style={{ left: `${eventPos}%`, background: '#29e6d3' }}
            title="開催日"
          />
        )}
          ...
          .map(t => {
            ...
            return (
              <div key={t.id} className="flex items-center gap-2 mb-1.5">
                <div className="w-40 shrink-0 text-xs text-gray-600 truncate" title={t.name}>
                  {t.name}
                </div>
                <div className="flex-1 relative h-5 bg-gray-50 rounded">
                  <div
                    className="absolute top-1 h-3 rounded-sm flex items-center px-1.5"
                    style={{
                      left: `${pos}%`,
                      width: '12px',
                      background: statusColor[t.status] || '#d1d5db',
      ...
      <div className="flex gap-4 mt-4 text-xs text-gray-400">
        ...
        <span className="flex items-center gap-1"><span className="w-3 h-2 rounded-sm inline-block" style={{ background: '#ef4444' }} />期限超過</span>
